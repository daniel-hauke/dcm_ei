function [f] = dde23_fx_nmda_RA(x,u,P,M,xtau,t)
...
try x_tau_di = spm_unvec(xtau(:,1),M.x); end         % DH ADDED
try x_tau_de = spm_unvec(xtau(:,2),M.x); end         % DH ADDED
...
SA      = sparse([1 0 1;      % SA must be specified here not in priors  
                  0 1 1;
                  0 0 0]);
...
% Check if global NMDA parameter should be estimated or not             
if isfield(P,'SNMDA')
   SNMDA = exp(P.SNMDA)*SNMDA; % scaling of NMDA receptor function everywhere  - RA added
end

% SNMDA(2,2) = exp(P.SNMDA(1))*SNMDA(2,2); % scaling of NMDA bkwd -> ii
% SNMDA(3,2) = exp(P.SNMDA(2))*SNMDA(3,2); % scaling of NMDA bkwd -> pc
    
% intrinsic connection strengths
%==========================================================================
G       = exp(P.G);
% G([2 4 6]) = G([1 3 5]);   % symmetry constraint on G                       - RA added


% intrinsic connections (np x np) - excitatory
%----------------------------------------------------------------------
GE = [0   0   1/2;            % GE must be specified here not in priors  
      0   0   1;
      1/2 0   0  ];

% intrinsic connections (np x np) - inhibitory
%----------------------------------------------------------------------
GI = [0   1/4 0;              % GI must be specified here not in priors 
      0   0   0;
      0   1   0];

% Check if global scaling for I function everywhere should be estimated or not
if isfield(P,'GI')
    GI = exp(P.GI).*GI;           % scaling of I function everywhere              - RA added
end

% Check if global scaling for E function everywhere should be estimated or not
if isfield(P,'GE')
    GE = exp(P.GE).*GE;           % scaling of I function everywhere              - RA added
end
                
 
% rate constants (ns x np) (excitatory 4ms, inhibitory 16ms)
%--------------------------------------------------------------------------
KE    = 1000/4;                           % excitatory time constants        - RA changed, NB also change lines 234-6
KI    = 1000/16;                          % inhibitory time constants
KNMDA = exp(-P.T)*1000/100;               % excitatory rate constants (NMDA) - RA changed (was scaling KE)
% KNMDA([2 4 6]) = KNMDA([1 3 5]);          % symmetry constraint on T         - RA added
...
%--------------------------------------------------------------------------
m_no_delays = spm_Ncdf_jdw(x{1}(:,:,1),VR,Vx); % DH ADDED: Input without delays
...
for k = 1:nc
    a(:,k) = A{k}.*diff_source*m_ex(:,end) + A{k}.*same_source*m_no_delays(:,end);
end
...
%         E = G(i)*GE(j,:)*m_in(i,:)';
%         I =      GI(j,:)*m_in(i,:)';
        E =      GE(j,:).*diff_pop(j,:)*m_in(i,:)' +      GE(j,:).*same_pop(j,:)*m_no_delays(i,:)'; % DH ADDED - RA changed
        I = G(i)*GI(j,:).*diff_pop(j,:)*m_in(i,:)' + G(i)*GI(j,:).*same_pop(j,:)*m_no_delays(i,:)'; % DH ADDED - RA changed (G was scaling GE)
        ...
        % Conductances
        %------------------------------------------------------------------
        f{1}(i,j,2) = (E - x{1}(i,j,2))*KE;                                 % RA changed
        f{1}(i,j,3) = (I - x{1}(i,j,3))*KI;
        f{1}(i,j,4) = (ENMDA' - x{1}(i,j,4))*KNMDA(i) ;                     % RA changed
...
% % Jacobian
% %==========================================================================
% J = spm_cat(spm_diff(M.f,xin,u,P,M,1));
% 

function [f] = dde23_fx_nmda(x,u,P,M,xtau,t)
...
%         E = G(i)*GE(j,:)*m_in(i,:)';
%         I =      GI(j,:)*m_in(i,:)';
        E = G(i)*GE(j,:).*diff_pop(j,:)*m_in(i,:)' + G(i)*GE(j,:).*same_pop(j,:)*m_no_delays(i,:)';
        I =      GI(j,:).*diff_pop(j,:)*m_in(i,:)' +      GI(j,:).*same_pop(j,:)*m_no_delays(i,:)';

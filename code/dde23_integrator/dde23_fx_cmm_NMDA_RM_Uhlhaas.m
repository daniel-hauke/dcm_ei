function [f] = dde23_fx_cmm_NMDA_RM_Uhlhaas(x,u,P,M,xtau,t)
...
%--------------------------------------------------------------------------
% P.GN_intrin([2 4 6])  = P.GN_intrin([1 3 5]);            % RA ADDED, orig [1 3 5] and [2 4 6] for 3 level model
% P.GGi_intrin([2 4 6]) = P.GGi_intrin([1 3 5]);
...
%         ENMDA  = (GN*exp(P.GN_intrin))*m(i,:)'    + (GN_toI*exp(P.GNi_intrin))*m(i,:)';
%         I      = (GI*exp(P.GG_intrin))*m(i,:)'    + (GI_B*exp(P.GGi_intrin))*m(i,:)';
        ...
        ENMDA  = (GN*exp(P.GN_intrin).*diff_pop)*m_in(i,:)' + (GN*exp(P.GN_intrin).*same_pop)*m_no_delays(i,:)' + ...     % DH CHANGED: Added delayed states
        ...
        I      = (GI*exp(P.GG_intrin).*diff_pop)*m_in(i,:)' + (GI*exp(P.GG_intrin).*same_pop)*m_no_delays(i,:)' + ...           % DH CHANGED: Added delayed states
                 (GI_B*exp(P.GGi_intrin).*diff_pop)*m_in(i,:)' + (GI_B*exp(P.GGi_intrin).*same_pop)*m_no_delays(i,:)' ;

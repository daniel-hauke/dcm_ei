...
%         I      = (G(:,:,i).*GI)*m(i,:)';
        ...
        E      = (G(:,:,i).*GE.*diff_pop)*m_in(i,:)' + (G(:,:,i).*GE.*same_pop)*m_no_delays(i,:)'; % DH CHANGED: Added delayed states
        ENMDA  = (G(:,:,i).*GE.*diff_pop)*m_in(i,:)' + (G(:,:,i).*GE.*same_pop)*m_no_delays(i,:)'; % DH CHANGED: Added delayed states

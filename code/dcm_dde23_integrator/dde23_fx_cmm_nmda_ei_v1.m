function [f] = dde23_fx_cmm_nmda_ei_v1(x,u,P,M,xtau,t)
...
if nargin < 5
    x_tau_de = reshape(x,ns,np,nk);   % DH ADDED
    x_tau_di = reshape(x,ns,np,nk);   % DH ADDED
end


% implement equality constraints across regions
%==========================================================================
P.T = repmat(P.T(1,:), ns,1); % same tau across all regions
...
GE_ampa = [ 0                    0                     0    0
            4*exp(P.g_se_ampa)   0                     0    0        % DH added: Scaling of intrinsic AMPA connection SS -> SP
            4                    4*exp(P.g_ei_ampa)    0    2        % DH added: Scaling of intrinsic AMPA connection SP -> II
            0                    4                     0    0];
     
GE_nmda = [ 0                    0                     0    0
            4*exp(P.g_se_nmda)   0                     0    0        % DH added: Scaling of intrinsic NMDA connection SS -> SP
            4                    4*exp(P.g_ei_nmda)    0    2        % DH added: Scaling of intrinsic NMDA connection SP -> II
            0                    4                     0    0];
...
GI   = [ 8    0                     2                      0               
         0    8*exp(P.g_ee_gaba)    2*exp(P.g_ie_gaba)     0          % DH added: Scaling self-inhibition of SP and scaling of GABA connection II -> SP
         0    0                    32*exp(P.g_ii_gaba)     0           % DH added: Scaling self-inhibition of II 
         0    0                     8                    128];
...
%         I      = (G(:,:,i).*GI)*m(i,:)';
        ...
        E      = (G(:,:,i).*GE_ampa.*diff_pop)*m_in(i,:)' + (G(:,:,i).*GE_ampa.*same_pop)*m_no_delays(i,:)'; % DH CHANGED: Added delayed states
        ENMDA  = (G(:,:,i).*GE_nmda.*diff_pop)*m_in(i,:)' + (G(:,:,i).*GE_nmda.*same_pop)*m_no_delays(i,:)'; % DH CHANGED: Added delayed states
        I      = (G(:,:,i).*GI.*diff_pop)*m_in(i,:)' + (G(:,:,i).*GI.*same_pop)*m_no_delays(i,:)'; % DH CHANGED: Added delayed states      

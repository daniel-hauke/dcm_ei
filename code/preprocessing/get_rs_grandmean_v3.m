...
opt.group_fname{1} = 'Subjects_BSNIP_Master.xlsx';          % Excel file with group codes
opt.group_col{1} = [5];                                   % Columns in excel file that have group code
opt.group_names{1} = {'HC'};                         % Group names (should match columns)
...
switch uid(1:end-1)
    ...
    case  'dell-cvmyz84\daniel'
        ...
        opt.pdata = 'E:\BSNIP\rest\results\rsEEG_preprocessed_final_files_60_chan_v3';
        opt.presults = 'D:\BSNIP\rest\results\rest_grandmean_v3';
...
for i = 1:numel(opt.group_fname)
    ...
    for g = 1:numel(opt.group_names{i})
        ...
        for s = 1:numel(ids)
            ...
            merged_file = fullfile(opt.pdata,'dcm_files_eo_ec',sprintf('dcm_%04d_prep_merged_eo_ec.mat',ids(s)));
            ...
            try
                ...
                warning('There was a problem with id: %04d',ids(s));
                warning('Error message: %s', err.message);
            end
        end
    end
end
